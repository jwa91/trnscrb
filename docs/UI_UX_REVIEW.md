...
- **B.** Add hover background highlight on rows (e.g. `.background(isHovered ? Color.primary.opacity(0.04) : .clear)`). Currently only the _selected_ row gets a background.
...
- **A.** Establish a consistent toolbar/header bar pattern for _both_ views. The main view should also have a top header bar (e.g. "trnscrb" title left, gear icon right) instead of burying the gear in the footer.
...
**Finding:** `chooseFilesButton` uses `.buttonStyle(.borderless)` + `.font(.caption)`. It renders as tiny plain blue text. In the compact mode, "Choose..." is even smaller. This is the _primary alternative_ to drag-and-drop and it looks like a footnote.
...
**Finding:** "Test" buttons for S3 and Mistral test with the _currently saved_ config or the _currently entered_ config? The answer is: the currently entered values, because the ViewModel passes them. But this isn't communicated. If you type a new API key and hit Test before Save, does it test the new key?
...
- **A.** Move settings access to a header bar (see point 5). The bottom-right corner is the _least_ scanned area of a popover.
...
- **B.** Keep the clipboard toggle as an _additional_ convenience on top of file saving.
